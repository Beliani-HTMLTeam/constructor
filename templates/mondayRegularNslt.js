...
function generateCategoriesSection(
  ...
  categories.forEach((category, index) => {
    ...
    // Генерируем HTML для всех картинок из productImg, если они есть
    let productImgHTML = "";
    if (Array.isArray(category.productImg) && category.productImg.length > 0) {
      productImgHTML = Create2Columns_Grid({
        ...
        iter: category.productImg.map((imgObj) => {
          // Если src — объект с value, берем value, иначе берем src напрямую
          return {
            ...imgObj,
              ...
              <tr>
               ...
               ${
                 under_intro_line
                   ? `
                <tr>
                <td class="newsletterContainer">${Line()}</td>
                </tr>
                 `
                   : ""
               }
                    ...
                    <td class="newsletterContainer" style="background-color: ${intro.background || background};">
                        ${Intro({
                          ...
                          type: categories.some(
                            (cat) =>
                              (cat.products && cat.products.length > 0) || (cat.productImg && cat.productImg.length > 0)
                          )
                            ? undefined
                            : "paragraph",
