...
const enrichProductEntry = async ({ product, getProductById, getCategoryLink }) => {
  try {
    ...
    const fetchedProduct = await getProductById(
      product.id,
      product.src,
      product.useDescription ? { useDescription: true } : undefined
    );
...
const normalizeCategoryForRender = ({ category, index, queries, getCategoryTitle, getCategoryLink, add_utm, country, type }) => {
  ...
  const ctaText = category?.cta?.textOverrides?.[countrySlug] ?? queries?.categoryButton?.[index] ?? null;
...
export const CategoriesHandler = async ({
  ...
  return Categories({
    ...
    theme,
    disableHighPrice,
    prodSettings: categories?.[0]?.prodSettings ?? {},
  });
};
