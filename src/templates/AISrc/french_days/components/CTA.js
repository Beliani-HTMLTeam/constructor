...
const CTA = ({
  ...
  if (src || alwaysRenderAsImage) {
    ...
    if (variant === 'cream') {
      ...
      const codeStyle = responsiveCode ? 'display:inline-flex;align-items:center;justify-content:center;text-align:center;font-size:22px;font-size:clamp(14px, 3.5vw, 22px);line-height:1.25;max-width:calc(100% - 48px);' : '';
      const label = responsiveCode ? `<span style="min-width:0;overflow-wrap:anywhere;">${text}</span>` : text;
      const positionedCopyIcon = responsiveCode ? `<span style="display:inline-flex;line-height:0;">${copyIcon}</span>` : copyIcon;
