...
export const renderOfferSection = ({ queries = {}, renderType = 'newsletter', links = {}, getPhrase = (text) => text, theme = {} }) => {
  ...
  const tiers = [1, 2, 3].map((tier) => {
    ...
    const codeButton = renderType === 'landing' && code ? CTA({ text: rawCode, codeValue: code, type: renderType, getPhrase, color: accent, bg: background, theme, variant: 'plain' }) : '';
  ...
  const button = renderType === 'newsletter' ? row(CTA({ href: links.TopImageTitle_href || links.TopImage_href || '#', text: getPhrase('Get code'), type: renderType, color: '#ffffff', bg: accent, borderColor: accent, borderWidth: '15px 30px', theme }), 'padding-top:30px;') : '';
