...
const getInnerTablePadding = (columns, columnId) => {
  ...
  if (columns !== 2) return '';
  return columnId === 0 ? 'padding-right: 10px;' : 'padding-left: 10px;';
};
...
const renderFreebieCard = ({ product, color, bg = '', freeText, fallbackHref, columns, columnId, settings = {} }) => {
            ...
            <td align="${alignConfig.contentTdAlign}">
              <table cellspacing="0" cellpadding="0" align="${alignConfig.innerCardAlign}" style="width: 100%; max-width: ${FREEBIE_CARD_MAX_WIDTH}px;">
                <tbody>
                  ...
                  <tr>
                    <td align="${settings.align}" ${bg ? `style="background-color: ${bg}; padding-left: 15px;"` : ''}>
                      <table cellspacing="0" cellpadding="0" style="width: 100%;">
                        <tbody>
                          ...
                          <tr>
                            <td align="${settings.align}" style="padding-top: 0px; padding-left: 0px; padding-right: 0px; padding-bottom: 0px; color: ${color}">
                              <span class="newsletterProductTitleFreebie">${productName}</span>
                            </td>
                          </tr>
                          ${productSize ? `<tr>
                            <td align="${settings.align}" style="color: ${color}">
                              <span class="newsletterProductTitleFreebie">${productSize}</span>
                            </td>
                          </tr>` : ''}
                          <tr>
                            <td align="${settings.align}" style="color: ${color}">
                              <span class="newsletterProductLowPriceFreebie">${freeText} </span>
                              <span class="newsletterProductHighPriceFreebie">${oldPrice}</span>
                            </td>
                          </tr>
                          <tr>
                            <td class="newsletterBottom40px"></td>
                          </tr>
                        </tbody>
                      </table>
                    </td>
...
export const renderFreebieGrid = ({ freebies, products, color, freeText, categoryHref, freebiesPerRow = 2, settings = {} }) => {
  ...
  for (let rowId = 0; rowId < rows.length; rowId++) {
    ...
    rowsHtml += `
      ${Space({ insideTable: true, className: rowId === 0 ? 'newsletterBottom40px' : '', bg: '#ffffff' })}
