import { ImageWithLink } from './ImageWithLink.js';
import { Prices } from './Prices.js';
import { Space } from './Space.js';

const Product = (
  product,
  showPrices,
  showName,
  color,
  align = 'left',
  priceColors = {},
  productsInnerBg = '',
  innerSpace = '',
  gapBetweenVertical = true,
  useCategoryLink = false,
  imageAlign = 'center'
) => {

  console.log(innerSpace, 'innerSpace');
  if (!product || typeof product !== 'object') return '';

  const resolveGapClass = (gap, fallback) => {
    if (gap === false || gap === 0) return '';
    if (typeof gap === 'string' && gap.trim()) return gap;
    return fallback;
  };

  const nameGapClass = resolveGapClass(gapBetweenVertical, 'newsletterBottom20px');
  const bottomGapClass =
    typeof gapBetweenVertical === 'string'
      ? gapBetweenVertical
      : resolveGapClass(gapBetweenVertical, product.spaceAfter ?? 'newsletterBottom30px');

  let html;

  if (productsInnerBg) {
    html = `
    <table cellspacing="0" cellpadding="0" border="0" width="100%">
      <tr>
        <td style="background-color: ${productsInnerBg};">
          <table cellspacing="0" cellpadding="0" border="0" width="100%">
    `;
  } else {
    html = `
    <table cellspacing="0" cellpadding="0" border="0" width="100%">
    `;
  }

  if (product.src && product.href) {
    html += ImageWithLink({
      href: useCategoryLink ? product?.categoryLink : product.href,
      src: typeof product.src === 'object' ? product.src.src : product.src,
      insideTr: true,
      align: imageAlign,
    });
  }
  let className = 'newsletterContainer15px';
  let exportPx = null;
  exportPx = className.replace(/\D/g, '');
  if (showName && product.name) {
    html += `
      ${nameGapClass ? Space({ insideTr: true, className: nameGapClass }) : ''}
      
      <tr>
        <td align="${align}" style="text-align: ${align}; color: ${color}; ${exportPx !== null ? `padding-left: ${exportPx}px; padding-right: ${exportPx}px;` : ''}" class="newsletterProductTitle ${className}">${product.name}</td>
      </tr>
    `;
  }

  if (showPrices && (product.lowPrice || product.highPrice)) {
    html += `
      ${Space({ insideTr: true, className: 'newsletterBottom10px' })}
      <tr>
        <td>
          ${Prices({
            high: product.highPrice || '',
            low: product.lowPrice || '',
            insideTr: true,
            color: color,
            priceColors: priceColors,
            align,
            addClass: 'newsletterContainer15px',
          })}
        </td>
      </tr>
    `;
  }
  html += `${innerSpace ? Space({ insideTr: true, className: innerSpace, bg: productsInnerBg }) : ''}`
  if (productsInnerBg) {
    html += `
          </table>
        </td>
      </tr>
    </table>
    ${bottomGapClass ? Space({ insideTable: true, className: bottomGapClass }) : ''}
    `;
  } else {
    html += `</table>`;
  }

  return html;
};

export { Product };
