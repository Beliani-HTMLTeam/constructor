...
const renderCategory = async (category, id, queries, getPhrase, getCategoryLink, getCategoryTitle, add_utm, links, type, country, categoryImageTdClass) => {
    ...
    <tr>
      <td>
        ${Paragraph({
          ...
          insideTable: true,
          spanStyle: `color: ${category?.titleColor ?? color};`,
          tableContainer: true,
      ...
      <tr>
        <td>
          ${Paragraph({
            ...
            className: 'newsletterCategoryParagraph',
            tableContainer: true,
            containerClass: 'newsletterContainer30px',
          })}
        </td>
  ...
  const ProductsElement =
    category.products || category.tiles || category.freebies
      ? await renderBody({
          ...
          offerBgColor: category.offerBgColor ?? '',
          product: category.product ?? {},
          productsOuterBg: category.productsOuterBg ?? '',
          productsInnerBg: category.productsInnerBg ?? '',
          productsInnerSpace: category.productsInnerSpace ?? '',
          priceColors: category.product?.priceColors ?? '',
          cta: category?.cta ?? {},
        })
      : '';
  ...
  const CTAElement = category.cta
    ? CTA({
        ...
        className: 'newsletterCategoryCta',
        variant: category.cta?.variant ?? 'underline',
        bgColor: category.cta?.bgColor ?? '#FF2D00',
        textColor: category.cta?.textColor ?? '#ffffff',
      })
    : '';
  ...
  <tr>
    <td>
      <table style="${styles}" cellspacing="0" cellpadding="0" border="0" width="100%">
        ...
        ${category.cta?.position === 'beforeProducts'
          ? `
          ${CTAElement}
        ...
        ${category.cta?.position === 'afterProducts'
          ? `
          ${CTAElement}
          ${category?.cta?.spaceAfter ? Space({ insideTr: true, className: category.cta.spaceAfter }) : ''}
          `
...
const renderBody = async ({
  ...
  try {
    ...
    return module.render({
      ...
      product,
      productsOuterBg: productsOuterBg,
      productsInnerBg: productsInnerBg,
      productsInnerSpace: productsInnerSpace,
