import { ImageWithLink, Space, Paragraph, Line, Timer, Category } from '@/components/index.js';
import { Footer } from '@/components/footer.js';
import { Header } from '@/components/header.js';

export function FridayEolProducts({
  links,
  getProductById,
  getCategoryLink,
  getCategoryTitle,
          ...
          <tbody>
              ...
              <tr>
                <td align="center" class="newsletterContainer">
                    ${Paragraph(queries.intro, 'center', `color: ${intro.color}`)}
                </td>
              </tr>
              ...
              <tr>
                <td align="center">
                    <a href="${
                      links.CTAcategory ? getCategoryLink(links.CTAcategory) : getPhrase('Shop now')
                    }" style="color: ${intro.color}; text-decoration: underline;">
                        <span class="newsletterCta" >${queries.CTAintro}</span>
                    </a>
                </td>
              </tr>
              ...
              ${
                timer && timer[country]
                  ? `
              <tr>
                <td style="color: ${timer?.color || '#000'}; background-color: ${timer?.background || background};">
                  ${Timer({
                    title: queries?.timer ? queries.timer[0] : '',
                    subtitle: queries?.timer ? queries.timer[1] : '',
                    href: links.Timer_href,
                    imageSrc: timer[country],
                    cta: getPhrase('Shop now'),
                    insidehref: links.Timer_href,
                    insidesrc: links.TimerFrebie,
                  })}
                </td>
              </tr>
                  `
              ...
              <tr>
                <td align="center">
                    ${Space({ className: 'newsletterBottom25px' })}
                </td>
              </tr>
              ${categories
                .map((category, index) => {
                  const isLast = index === categories.length - 1;
                  const catBackground = category.background || background;
                  const catColor = category.color || '#fff';
                  
                  // Get products with their data
                  const categoryProducts = category.products.map((prod) => 
                    getProductById(prod.id, prod.src)
                  );
                  
                  // Determine the correct href for the category
                  const categoryHref = category.href && category.href !== '' 
                    ? getCategoryLink(category.href)
                    : links.CTAcategory ? getCategoryLink(links.CTAcategory) : getPhrase('Shop now');
                  
                  return `
              <tr>
                <td style="background-color: ${catBackground}; color: ${catColor};">
                  ${Category({
                    data: queries?.categories && queries.categories[index * 2] 
                      ? [queries.categories[index * 2], queries.categories[index * 2 + 1]]
                      : [category.name, ''],
                    href: categoryHref,
                    name:
                    ...
                    align: 'left',
                    src: category.src,
                    cta: queries?.CTAcategory ? queries.CTAcategory : getPhrase('Shop now'),
                    color: catColor,
                    type: category.type || 'EOL',
