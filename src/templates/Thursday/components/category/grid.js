...
export const render = (products, showPrices, showNames, queries, insideContainer = true) => {
  ...
  if (Array.isArray(products)) {
    ...
    for (let i = 0; i < products.length; i += cols) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        // prettier-ignore
        productsInnerHtml += `<td width="50%" ${(c + 1) % 2 !== 0 ? 'class="newsletterRight10px"' : 'class="newsletterLeft10px"'}>`;
