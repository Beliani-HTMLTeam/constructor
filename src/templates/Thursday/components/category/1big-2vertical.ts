...
/**
 * `type: '1big-2vertical'` — groups of 3 products where the first is rendered "big"
 * (full-width row) and the next two stack in a narrow side column, alternating which
 * side the big product sits on every group.
 */
export const render = ({
  products,
  showPrices,
  ...
  const renderGapTd = (side: 'left' | 'right'): string => {
  ...
  let imgSide = imageSide ?? ((id ?? 0) % 2 === 0 ? 'left' : 'right');
  ...
  if (Array.isArray(products)) {
    ...
    for (let i = 0; i + 2 < products.length; i += 3) {
      ...
      const bigProduct = (p?: ProductEntry): string => {
        const withCategoryLink = p?.useCategoryLink ? { ...p, categoryLink: categoryHref } : p;
        return Product(withCategoryLink as ProductEntry, showPrices ?? true, showNames ?? true, '#000', align, gapBetweenVertical, p?.useCategoryLink);
      };
      ...
      productsInnerHtml += `<tr><td>${Product(products[i + 1], showPrices ?? true, showNames ?? true, color ?? '#000000', align, gapBetweenVertical, products[i + 1]?.useCategoryLink)}</td></tr>`;
      productsInnerHtml += `<tr><td>${Product(products[i + 2], showPrices ?? true, showNames ?? true, color ?? '#000000', align, gapBetweenVertical, products[i + 2]?.useCategoryLink)}</td></tr>`;
