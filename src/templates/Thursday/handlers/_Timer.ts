...
import type { InsideConfig, QueriesConfig, LinksConfig, TimerConfig, RenderType } from '@/types/thursday';

let languagesMap: Record<string, { title?: string }> = {
  ...languages,
  // "HU":
  // {
  //   ...languages.HU,
...
export interface BuildProloTimerScriptProps {
  /** Deadline date, formatted `YYYY-MM-DD`. */
  deadline: string;
  /** Current market/language code, used to resolve the IANA timezone and unit labels. */
  country: string;
}

/**
 * Builds a self-contained client-side `<script>` that renders a live, per-second
 * countdown into a `#prolo-timer` element (used on the landing-page surface, where
 * a static server-rendered GIF isn't appropriate). Converts `deadline` (a wall-clock
 * date, end-of-day in the market's own timezone) to a UTC epoch once at build time,
 * so the shipped script only does simple arithmetic against `Date.now()`.
 *
 * @returns A `<script>...</script>` HTML fragment, or `''` if `deadline` fails to parse.
 */
const buildProloTimerScript = ({ deadline, country }: BuildProloTimerScriptProps): string => {
  const timerLabels = labels[country as keyof typeof labels] ?? labels.UK;
  const timezone = timezones[country as keyof typeof timezones];

  const wallClock = `${deadline}T23:59:00`;

  function wallClockToUtcMs(wc: string, tz: string): number {
...
export interface TimerHandlerProps {
  /** Countdown section visual config. */
  Inside?: InsideConfig;
  /** Resolved translation queries; `queries.timer` supplies `[title, subtitle]`. */
  queries: QueriesConfig;
  /** Campaign links map; `links.Timer_href` is the countdown's link target. */
  links: LinksConfig;
  /** Countdown deadline/freebies data. */
  timer?: TimerConfig;
  ...
  type: RenderType;
  /** Shop record; `origin` seeds the countdown-GIF base URL, `slug` special-cases the PT locale. */
  shop: { origin?: string; slug?: string };
}

/**
 * Wires campaign `Inside`/`timer` config into `Timer` (`components/ProloTimer.ts`).
 * Renders nothing unless `Inside.type === 'timer'` and `timer.deadline` is set. Builds
 * the Prologistics countdown-GIF URL (newsletter surface) and the live countdown
 * `<script>` (landing-page surface, via `buildProloTimerScript`) in one pass.
 */
export const TimerHandler = ({ Inside, queries, links, timer, shopNow, country, type, shop }: TimerHandlerProps): string => {
  ...
  let link = base + `?deadline=${timer.deadline}T23:59:00` +
    `&timezone=${timezones[country as keyof typeof timezones]}` +
  ...
  let freebies: string | null = null;

  if (timer?.freebies) {

    if (typeof timer.freebies === 'string') {
      freebies = timer.freebies;
    }
    else {
      freebies = timer.freebies.src;
    }
  }
