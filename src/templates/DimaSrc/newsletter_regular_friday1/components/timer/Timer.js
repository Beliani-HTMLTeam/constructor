...
export const Timer = async ({
  ...
  console.log("timer", timer.overrides[country], country, config);
  ...
  let freebies = timer?.freebies || null;
  
  // Check for country-specific freebies override
  if (timer?.overrides && timer.overrides[country]) {
    // If the override is a string, use it as the image URL
    if (typeof timer.overrides[country] === 'string') {
      // You might need to call getImageUrl here if it's a filename
      // or use it directly if it's already a full URL
      freebies = getImageUrl(timer.overrides[country], true);
    } 
    // If the override is an object and has freebies property
    else if (timer.overrides[country].freebies) {
      freebies = getImageUrl(timer.overrides[country].freebies, true);
    }
  }
