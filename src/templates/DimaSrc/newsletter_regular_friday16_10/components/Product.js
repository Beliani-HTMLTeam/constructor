...
const Product = (product, showPrices, showName, color, priceColor, idx, isSpaceAfter = true, priceAdditionalPadding = true) => {
  ...
  if (showPrices && (product.lowPrice || product.highPrice)) {
    html += `
      <tr>
        <td style="color: ${color || '#000000'}; text-align: center;" class="${idx % 2 === 0 && priceAdditionalPadding ? 'newsletterLeft20px' : ''}">
          ${Prices({
            high: product.highPrice || '',
            low: product.lowPrice || '',
            color: priceColor ||color || '#000000',
          })}
        </td>
      </tr>
    `;
