...
function generateCategoriesSection(
  ...
  categories.forEach((category, index) => {
    ...
    if (category.products && category.products.length === 2) {
      categoryType = 'only 2 products in category';
    }
    ...
    categoriesHTML += `
      <tr>
        <td style="background-color: ${category?.background || background};">
          ${
            (index === 0 && categoryType === 'monday') || categoryType === 'productImg'
              ? Space({
                  className:
    ...
    categoriesHTML += `
      <tr>
        <td style="background-color: ${category?.background || background}; color: ${
...
export async function customisedMondayRegularNSLT({
  links,
  getProductById,
  getCategoryLink,
  getCategoryTitle,
  ...
  <table cellspacing="0" cellpadding="0" border="0" align="center" width="100%" style="max-width: 650px; width: 100%; background-color: ${background}; color: #000;" id="newsletter">
        <tbody>
              ...
              `
                  : `<tr>
                  <td style="background-color: ${inside.background || background}; color: ${
                      inside.color || '#000'
                    };">
                    ${Space()}
                    ${ImageWithLink({
