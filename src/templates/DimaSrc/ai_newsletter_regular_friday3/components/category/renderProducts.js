...
export const renderProducts = async (
 { products,
  ...
  background,
  color = '#000000', country, category}
) => {
  const type = categoryType ? categoryType.toLowerCase() : 'default';

  console.log('renderProducts', type, "products in renderProducts", products);
  ...
  try {
    ...
    if (type === 'unique') {
      ...
      return module.render({products, showPrices, showNames: showName, queries, insideContainer, color, background, category});
      ...
      return module.render(products, showPrices, showName, queries, insideContainer, color, background, category);
    }
