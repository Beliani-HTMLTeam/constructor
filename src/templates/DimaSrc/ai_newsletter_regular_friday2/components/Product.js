...
const Product = ({product, showPrices, showName, color, idx, isSpaceAfter = true, priceAdditionalPadding = true, category}) => {
  ...
  console.log("category", category, "product", product, "showPrices", showPrices, "showName", showName, "color", color);
  ...
  if (showPrices && (product.lowPrice || product.highPrice)) {
    html += `
      <tr>
        <td style="color: ${color || '#000000'};" class="${idx % 2 === 0 && priceAdditionalPadding ? 'newsletterLeft20px' : ''}">
          ${Prices({
