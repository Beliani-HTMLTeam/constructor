...
const AIRegularFridayNslt09_10 = async ({
  links,
  queries,
  shop,
  country,
  ...
  const {
    intro: introQueries, offerPart, timer: timerQueries
  } = queries;

  const {
    TopImage_src, TopImage_href, Timer_href, Banner_1, Banner_1_Image, Banner_2, Banner_2_Image, code_href, ShopCTA
  } = links;

  const { color: TiT_color, backgroundColor: TiT_bg, type: TiT_type, className: TiT_className } = TopImageTitle_data || {};

  const introTextContent = introQueries || introData?.text;
  const hasTopHtmlHeader = Boolean(introTextContent);
  ...
  return `
    ...
    <table cellspacing="0" cellpadding="0" border="0" align="center" width="100%" width="650" style="max-width: 650px; width: 100%; color: #000; background-color: ${background};" id="newsletter">
      ${TopImageElement}
...
export { AIRegularFridayNslt09_10 };
