...
export class MondayFreebieLanding extends LandingTemplate {
  ...
  constructor(
    ...
    if (!freebies) {
      throw new Error('freebies property should be defined in MondayFreebieLanding template');
    }
    if (!('items' in freebies)) {
      throw new Error('items is not defined in freebiew object.');
    }
    if (!categories) {
      throw new Error('categories should defined in MondayFreebieLanding template');
    }
